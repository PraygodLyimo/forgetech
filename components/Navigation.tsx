          ...
          <nav className="hidden md:flex gap-8">
            {navigationItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`relative font-semibold transition-colors duration-300 after:absolute after:left-0 after:-bottom-1.5 after:h-1 after:bg-[#ff6219] after:transition-all after:duration-300 ${isCurrentPage(item.href)
              ...
              <div className="space-y-4">
                {navigationItems.map((item, index) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`block font-semibold transition-colors py-3 px-4 rounded-lg border-l-4 ${isCurrentPage(item.href)
